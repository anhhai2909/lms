<%-- 
    Document   : ClassUpdate
    Created on : Oct 14, 2023, 2:26:04 PM
    Author     : OS
--%>
                    ...
                    <div class="col-lg-12 m-b30">
                        <div class="widget-box">
                            ...
                            <div class="widget-inner">
                                <form name="classListForm" id="classListForm" action="classes" method="get" class="edit-profile m-b30" onsubmit="<%= isLoggedIn ? "return validateAndSubmit();" : "return checkLoginStatus();"%>">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="form-group col-3">
                                            ...
                                            <div>
                                                <input class="form-control" type="text" name = "" value="${classes.getId()}" disabled>
                                            ...
                                            <div>
                                                <input type="hidden" name="is_active" value="${classes.getIs_active()}"> <!-- Hidden input with value from the database -->
                                                <label class="switch">
                                                    <input type="checkbox" name="is_active_checkbox" value="1" ${classes.getIs_active() == 1 ? 'checked' : ''}>
                                    ...
                                    <div class="row">
                                        <div class="form-group col-3">
                                            ...
                                            <div>
                                                <input class="form-control" type="text" name = "class_name" value="${classes.getClass_name()}">
                                                <span id="settingValueError" style="color: red;"></span>
                                            </div>
                                        </div>
<div class="form-group col-3">
    <label class="col-form-label" style="font-weight: 500;">Semester*</label>
    <div>
        <select class="form-control" name="semester_id">
            <c:forEach var="c" items="${requestScope.semesters}">
                <option value="${c.id}" ${c.id eq classes.getSemester_id() ? 'selected' : ''}>${c.setting_value}</option>
            </c:forEach>
        </select>
    </div>
</div>
                                                ...
                                                <div class="row">
                                        <div class="form-group col-3">
                                            <label class="col-form-label" style="font-weight: 500;">Teacher*</label>
                                            <div>
                                                <select class="form-control" name="trainer_id">
                                                    <c:forEach var="c" items="${requestScope.trainers}">
                                                        <option value="${c.id}" ${c.id eq classes.getTrainer_id() ? 'selected' : ''}>${c.fullname}</option>
                                                    </c:forEach>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-group col-3">
                                            <label class="col-form-label" style="font-weight: 500;">Subject*</label>
                                            <div>
                                                <select class="form-control" name="subject_id">
                                                    <c:forEach var="c" items="${requestScope.subjects}">
                                                        <option value="${c.id}" ${c.id eq classes.getSubject_id() ? 'selected' : ''}>${c.code}</option>
                                                    </c:forEach>
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    <div class="row">
<div class="form-group col-3">
    <label class="col-form-label" style="font-weight: 500;">Start date* (month/day/year)</label>
    <div>
        <fmt:formatDate pattern="yyyy-MM-dd" value="${classes.start_date}" var="formattedStartDate" />
        <input class="form-control" type="date" name="start_date" value="${formattedStartDate}">
    </div>
</div>
<div class="form-group col-3">
    <label class="col-form-label" style="font-weight: 500;">End date* (month/day/year)</label>
    <div>
        <fmt:formatDate pattern="yyyy-MM-dd" value="${classes.end_date}" var="formattedEndDate" />
        <input class="form-control" type="date" name="end_date" value="${formattedEndDate}">
    </div>
</div>
                                        <div class="form-group col-2"></div>
                                    </div>
                                    <div class="row">
                                        <div class="form-group col-6">
                                            ...
                                            <div>
                                                <textarea class="form-control" name = "description" rows="4">${classes.getDescription()}</textarea>
                                            </div>
                                        </div>
                                    </div>
    ...
    <div class="ttr-overlay"></div>
                                        <script>
                                            function goToDetail() {
                                                window.location.href = "classes?action=detailScreen&id=${classes.getId()}";
                                            }
                                        </script>
...
function validateForm() {
    var class_name = document.forms["classListForm"]["class_name"].value;
    var start_date = document.forms["classListForm"]["start_date"].value;
    var end_date = document.forms["classListForm"]["end_date"].value;
